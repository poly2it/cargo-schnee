...
let
  ...
  # cargo-schnee's clippy subcommand has no `--workspace` flag — its
  # default behaviour (no `-p`) already lints every workspace member.
  # Strip `--workspace` from the consumer's scope so existing call
  # sites continue to evaluate; semantics match.  `--all-targets` is
  # forwarded through cargoExtraArgs to cargo-schnee, which now
  # recognises it and extends the cargo CompileFilter accordingly.
  filteredScope =
    lib.filter (a: a != "--workspace") effectiveScope;
  preCargoArgs = filteredScope ++ clippyExtraArgs;
